package wizard;
/**
 * found at : http://java.sun.com/developer/technicalArticles/GUI/swing/wizard/
 *
 */
...
/**
 * r.nagel
 * insert a last-button-clicked flag for detecting the direction of wizard control
 */
...
public class Wizard extends WindowAdapter implements PropertyChangeListener
{
  ...
  public static final int
      LAST_BUTTON_NOTHING = 1,
      LAST_BUTTON_BACK = 2,
      LAST_BUTTON_NEXT = 3 ;

  /** status of buttons */
  private int lastButtonClicked = LAST_BUTTON_NOTHING ;
   ...
   */
  public static final String FINISH_BUTTON_ACTION_COMMAND =
      "FinishButtonActionCommand" ;
  ...
  public Wizard( boolean buttonStyle)
  {
    wizardModel = new WizardModel( buttonStyle ) ;
  ...
  public int showModalDialog()
  {
    ...
    wizardDialog.pack() ;
    wizardDialog.setVisible(true);  // make it Java 1.5 ready
//    wizardDialog.show() ;
  ...
  public void propertyChange( PropertyChangeEvent evt )
  {
    ...
    if ( evt.getPropertyName().equals( WizardModel.
                                       CURRENT_PANEL_DESCRIPTOR_PROPERTY ) )
    {
      wizardController.resetButtonsToPanelRules() ;
    }
    else if ( evt.getPropertyName().equals( WizardModel.
    ...
    {
      finishButton.setText( evt.getNewValue().toString() ) ;
    }
    else if ( evt.getPropertyName().equals( WizardModel.
                                            NEXT_FINISH_BUTTON_ENABLED_PROPERTY ) )
    {
      nextButton.setEnabled( ( ( Boolean ) evt.getNewValue() ).booleanValue() ) ;
                                            ...
                                            FINISH_BUTTON_ENABLED_PROPERTY ) )
    {
      finishButton.setEnabled( ( ( Boolean ) evt.getNewValue() ).booleanValue() ) ;
    }
    else if ( evt.getPropertyName().equals( WizardModel.
                                            ...
                                            CANCEL_BUTTON_ICON_PROPERTY ) )
    {
      cancelButton.setIcon( ( Icon ) evt.getNewValue() ) ;
                                            ...
                                            FINISH_BUTTON_ICON_PROPERTY ) )
    {
      finishButton.setIcon( ( Icon ) evt.getNewValue() ) ;
    }
...
//    backButton = new JButton( new ImageIcon( "com/nexes/wizard/backIcon.gif" ) ) ;
    ...
    finishButton.addActionListener( wizardController );
    ...
    if ( wizardModel.isExtraFinishButton())
    {
...
/*  private static Object getImage( String name )
  ...
  public void setLastButtonClicked( int pLastButtonStatus )
  {
    this.lastButtonClicked = pLastButtonStatus ;
  }
  // --------------------------------------------------------------------------

  // changed to be conform with popeye structure
  static
  {
    /*
            try {
    ...
    BACK_ICON = new ImageIcon( GUIGlobals.wizardPrevName ) ;
    NEXT_ICON = new ImageIcon( GUIGlobals.wizardNextName ) ;
    CANCEL_ICON = new ImageIcon( GUIGlobals.wizardCancelName ) ;
    FINISH_ICON = new ImageIcon( GUIGlobals.wizardOkName ) ;
  }
